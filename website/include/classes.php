...
class Application {
    ...
    public function auditlog($context, $message, $priority = 0, $userid = NULL){
		...
		$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/auditlog";
			$data = array(
				'context'=>$context,
				'message'=>$message,
				'logdate'=>$logdate,
        ...
        $ch = curl_init();
			...
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 186xUX3FnMEYQvwSqEn08fDuQaNSEE91EmxGKeRi'));
   ...
   public function register($username, $password, $email, $registrationcode, &$errors) {
        ...
        if (sizeof($errors) == 0) {
			...
			$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/RegisterUser";
			$data = array(
				'userid'=>$userid,
				'username'=>$username,
			...
			$data_json = json_encode($data);
			$apiKey = '186xUX3FnMEYQvwSqEn08fDuQaNSEE91EmxGKeRi';
			$headers = array('Authorization: '.$apiKey); 

			$ch = curl_init();
			curl_setopt($ch, CURLOPT_URL, $url);
			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ', $headers)); 
			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			$response  = curl_exec($ch);
    ...
    */
    ...
    /* protected function sendValidationEmail($userid, $email, &$errors) {
        ...
        if ($result === FALSE) {
            ...
            $message = "A request has been made to create an account at One Stop Photo for this email address. ".
     ...
     protected function sendValidationEmail($userid, $email, &$errors) {
		...
		$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/sendValidationEmail";
			$data = array(
    ...
    public function processEmailValidation($validationid, &$errors) {
     ...
     public function processOTPValidation($validationid, &$errors) {
  	...
  	*/
  	 public function newSession($userid, &$errors, $registrationcode = NULL) {
        ...
        if (sizeof($errors) == 0) {
            ...
            // Create a new session ID
            $sessionid = bin2hex(random_bytes(25));
			$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/newSession";
			$data = array(
    ...
    */
	...
	public function getUserRegistrations($userid, &$errors) {
		...
		$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/getUserRegistration?userid=".$userid;
   ...
   public function clearPasswordResetRecords() {
		...
		$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/clearPasswordReset?passwordresetid=" . $passwordresetid;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		...
		if ($response === FALSE) {
			...
			if($httpCode == 400) {
                ...
                $this->auditlog("clearPasswordResetRecords", "successful: $passwordresetid");
			}
		}
        ...
        if (sizeof($errors) == 0) {
            ...
            if ($result === FALSE) {
                ...
                if (!password_verify($password, $row['passwordhash'])) {
                    ...
                    $userid = $row['userid'];
                    $email = $row['email'];
                    $this->sendOTPEmail($userid,$email);
   ...
   /* public function logout() {
	...
	public function logout() {
		...
		$sessionid = $_COOKIE['sessionid'];
		$url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/logout?usersessionid=" . $sessionid;
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, $url);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE');
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    ...
    /*
     public function getThings(&$errors) {
        ...
        $url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/getThing?registrationcode=". $registrationcode;
    ...
    public function saveAttachment($dbh, $attachment, &$errors) {
     ...
     public function addThing($name, $attachment, &$errors) {
        ...
        if (sizeof($errors) == 0) {
            ...
            $url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/addThing";
            // Create a new ID
            ...
            if (sizeof($errors) == 0) {
                ...
                $data_json = json_encode($data);
          			$ch = curl_init();
          			curl_setopt($ch, CURLOPT_URL, $url);
          			curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), '186xUX3FnMEYQvwSqEn08fDuQaNSEE91EmxGKeRi'));
          			curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'POST');
          			curl_setopt($ch, CURLOPT_POSTFIELDS, $data_json);
          			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
          			$response  = curl_exec($ch);
        ...
        if(sizeof($errors)== 0) {
            ...
            if (!$isadmin && $loggedinuserid != $userid) {
                ...
                if (sizeof($errors) == 0) {
                  ...
                  $url = "https://c0mks5o0f7.execute-api.us-east-1.amazonaws.com/v1/getUser?userid=" . $userid;
                 $ch = curl_init();
                 curl_setopt($ch, CURLOPT_URL, $url);
                     curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json','Content-Length: ' . strlen($data_json), 'x-api-key: 186xUX3FnMEYQvwSqEn08fDuQaNSEE91EmxGKeRi'));
                 curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'GET');
